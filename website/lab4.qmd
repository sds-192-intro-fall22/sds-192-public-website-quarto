...
When you first create a repository all of the code will be stored in the *main* branch of the repository. If you were to think of a project like a tree growing up from the ground, then the main branch would be the like the trunk of the tree. One goal of a branching workflow in GitHub is to keep the most stable and polished versions of code in the main branch.

So what do we do in the meantime - when we're editing code and trying to sort out its bugs, and it's not quite in that stable and polished state yet? That's where branching comes in.

When we create a branch of our repo, GitHub creates a separate copy of the repo where you can make changes without impacting what's in the main branch. Later, once we're down making changes and things are stable and polised, we will have the opportunity to merge those changes back into the main branch.
...
If this seems confusing imagine this: let's say you write a paper, and you share it with two of your classmates to review. One peer reads through it, makes suggested changes to the opening sentence, sends it back to both you and the other classmate. Later that week the second the second classmate also makes changes to the opening sentence but without first looking at or incorporating the changes the first peer made. A pull error is like you trying to automatically incorporate your first classmate's edits. You're not sure what to do because your second classmate made changes in the same place. What should you do?

Well a first step might be just to be sure that you have a saved copy of the second classmate's suggested changes before incorporating the first classmate's suggestions. Then you can figure out later how to resolve the conflict. That's exactly what we are going to do here. To fix this error, you should stage and commit your local changes and then try pulling again. Let's replicate this error:
